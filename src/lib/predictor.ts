...
import type { CellInsight } from "@/lib/contracts";

interface RawCellStat {
  cellIndex: number;
...
export function buildCellInsights({ mineCount, totalRounds, totalLosses, stats }: InsightContext) {
  ...
  return getAllCells().map((cellIndex) => {
    ...
    const mineRate = (stat.mineReports + baselineMineRate * 4) / (totalLosses + 4);
